...
describe('links API', async () => {
  ...
  it('stores a link-level medium override and clears it back to the campaign', async () => {
    ...
    const link = await $fetch<{ id: string; slug: string; utmMedium: string | null }>('/api/links', {
      method: 'POST',
      body: { destinationUrl: 'https://example.com/medium', campaignId, utmMedium: 'email', slug: 'medium-clear' },
      headers: { cookie },
    });
    expect(link.utmMedium).toBe('email');

    const withOverride = await fetch('/medium-clear', { redirect: 'manual' });
    expect(withOverride.status).toBe(302);
    expect(new URL(withOverride.headers.get('location')!).searchParams.get('utm_medium')).toBe('email');
    ...
    const withCampaign = await fetch('/medium-clear', { redirect: 'manual' });
    expect(withCampaign.status).toBe(302);
    expect(new URL(withCampaign.headers.get('location')!).searchParams.get('utm_medium')).toBe('social');
  });
