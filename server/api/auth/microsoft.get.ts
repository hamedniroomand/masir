...
export default defineOAuthMicrosoftEventHandler({
  ...
  async onSuccess(event, { user }) {
    ...
    if (!result.ok) {
      await writeAuditEvent('oauth_refused', { provider: 'MICROSOFT', reason: result.reason });
      return sendRedirect(event, `/login?error=${encodeURIComponent(result.reason)}`);
    }
    await setSessionUser(event, result.user);
    await writeAuditEvent('oauth_login', { provider: 'MICROSOFT' }, { actor: result.user.id });
    return sendRedirect(event, '/');
  },
  async onError(event, error) {
    await writeAuditEvent('oauth_failed', { provider: 'MICROSOFT', message: error.message });
