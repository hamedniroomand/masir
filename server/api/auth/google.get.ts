...
export default defineOAuthGoogleEventHandler({
  ...
  async onSuccess(event, { user }) {
    ...
    if (!result.ok) {
      await writeSecurityEvent('oauth_refused', { provider: 'GOOGLE', reason: result.reason });
  ...
  async onError(event, error) {
    await writeSecurityEvent('oauth_failed', { provider: 'GOOGLE', message: error.message });
